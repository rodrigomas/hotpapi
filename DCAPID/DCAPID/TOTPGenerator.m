...
- (long) valueAtTime:(long) date
{
    long time = date;
    
    long timeSinceStartTime = time - 0; //mStartTime
    
    if (timeSinceStartTime >= 0) {
        return timeSinceStartTime / self.period;
    } else {
        return (timeSinceStartTime - (self.period - 1)) / self.period;
    }
}

- (long) startTimeWithValue:(long) value
{
    return 0 + (value * self.period); //mStartTime
}
